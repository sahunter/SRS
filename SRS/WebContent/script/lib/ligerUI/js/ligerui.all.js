...
(function ($)
{
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
            var g = this, p = this.options, lastVisitedGroup = null; 
            //分组： 先填充没有设置分组的字段，然后按照分组排序
            $(p.fields).each(function (i, field)
            {
                if (field.newline == null) field.newline = true;
                if (lastVisitedGroup && !field.group) field.group = lastVisitedGroup;
                lastVisitedGroup = field.group || lastVisitedGroup;
            });
            p.fields.sort(function (a, b)
            {
                if (!a.group) return false;
                if (!b.group) return true;
                if (typeof (a.group) != "string") a.group = a.group.toString();
...
(function ($)
{
    ...
    function getEditorBuilder(e)
    {
        ...
        return $.extend({
            ...
            {
                return liger.get(input).getValue();
            },
            ...
            {
                liger.get(input).resize(width, height);
            },
            destroy: function (input, editParm)
            {
                liger.get(input).destroy();
            }
        }, e); 
    }
    //几个默认的编辑器构造函数
        ...
        submitEdit: function (rowParm)
        {
            ...
            else
            {
                ...
                for (var columnid in g.editors[rowdata['__id']])
                {
                    ...
                    {
                        newdata[column.name] = o.editor.getText(o.input, o.editParm);
                    }
                }
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                g.bind('sysEndEdit', function ()
                {
                    ...
                    if (newValue != currentdata)
                    {
                        ...
                        if (column.textField && column.editor.getText)
                        {
                            editParm.text = column.editor.getText(editorInput, editParm);
                        }
